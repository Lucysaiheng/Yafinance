...
var training_data = full_data.filter(function(d) { return ((d.region=="northwest") || (d.region=="southwest")); });
var test_data = full_data.filter(function(d) { return ((d.region!="northwest") && (d.region!="southwest")); });
...
test_data.forEach(function(d) {
    ...
    if (prob_vec.northwest > prob_vec.southwest) {
        d.label = "northwest";
    }
    else if (prob_vec.southwest > prob_vec.northwest) {
        d.label = "southwest";
    }
    else {
        d.label = "north";
...
function renderVis(_subset) {
    ...
    var circles = svg.selectAll(".label").data(training_data, function(d) {return d.name;});
    circles.exit()
        .transition()
            .duration(250)
            .attr("r",0)
